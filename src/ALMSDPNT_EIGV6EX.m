function [X, S, y, fval] = ALMSDPNT_EIGV6EX(At, b, c, n)
...
A = At';
p = 2;
sigma = 1e-2; sigma_min=1e-2; sigma_max = 1e7;
...
%     [vS, dS] = mineig(S);      % for large eigen decomposition problem
    ...
    if pinf/dinf < eta1 && mineigS > tao
%        it_pinf = it_pinf + 1; it_dinf = 0;
%        if it_pinf >= h4
          sigma = max(sigma/gamma,sigma_min); %it_pinf = 0;
%        end
    elseif pinf/dinf > eta2 || pinf > 0.65*pinfold
