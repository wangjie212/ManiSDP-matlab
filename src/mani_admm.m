function [Y, fval] = mani_admm(SDP, At, b, c, mb, p, sb, sB, M1, M2, maniIter)
...
bsol = [];
bgap = [];
% while i <= maniIter && gap > 1e-6
y = zeros(length(b),1);
[Y, ~, ~, fval] = ALMSDP(At, b, c, mb, p, 1e-6, y, []);
% if isempty(bsol) || fval < best/(1-bgap/3)
%     [sol, gap] = DRS(sb, sB, M1, M2, mb, fval, [], bgap, 1e-6, 0);
%     best = fval;
%     bX = X;
%     bsol = sol;
%     bgap = gap;
...
% i = i + 1;
% end
[~, gap] = DRS(sb, sB, M1, M2, mb, fval, bsol, bgap, 1e-12, 1);
...
% X = aX{1};
% fval = obj(1);
disp(['ManiPOP: optimum = ' num2str(fval) ', gap <= ' num2str(gap) ', time = ' num2str(time) 's']);
end